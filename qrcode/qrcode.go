...
func (q *QRCode) encodeBlocks() *Bitset {
	...
	for _, b := range q.version.block {
		for j := 0; j < b.numBlocks; j++ {
			...
			block[blockID].data = EncodeBit(q.data.Substr(start, end), numErrorCodewords)
...
func (q *QRCode) addPadding() {
	...
	padding := [2]*Bitset{
		NewBit(true, true, true, false, true, true, false, false),
		NewBit(false, false, false, true, false, false, false, true),
	}
